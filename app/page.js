...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-[#D8D8D8] p-4">
        <div className="flex justify-center">
          <input
            type="text"
            placeholder="Your next big idea..."
            className="w-[700px] px-4 py-2 rounded-md focus:outline-none focus:border-blue-500"
            value={searchQuery}
      ...
      <main className="p-20 bg-[#F6F5F5]">
        <div className="grid grid-cols-6 gap-3">
          {filteredEntries.map((text, index) => (
            <div
              key={index}
              className="aspect-square bg-white rounded-md border-2 border-black p-5 overflow-scroll"
            >
              <p>{text}</p>
            </div>
          ))}
